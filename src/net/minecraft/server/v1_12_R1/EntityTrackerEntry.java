...
public class EntityTrackerEntry
{
    ...
    public EntityTrackerEntry(final Entity entity, final int i, final int j, final int k, final boolean flag) {
    ...
    public void track(final List<EntityHuman> list) {
        ...
        final List list2 = this.tracker.bF();
        ...
        if (this.tracker instanceof EntityItemFrame && this.a % 20 == 0) {
            ...
            if (itemstack != null && itemstack.getItem() instanceof ItemWorldMap) {
                ...
                for (final EntityHuman entityhuman : this.trackedPlayers) {
                    ...
                    final Packet packet = Items.FILLED_MAP.a(itemstack, this.tracker.world, (EntityHuman)entityplayer);
                    if (packet != null) {
        ...
        if (this.a % this.g == 0 || this.tracker.impulse || this.tracker.getDataWatcher().a()) {
            if (this.tracker.isPassenger()) {
                ...
                if (flag) {
                    this.broadcast((Packet<?>)new PacketPlayOutEntity.PacketPlayOutEntityLook(this.tracker.getId(), (byte)i, (byte)j, this.tracker.onGround));
                    this.yRot = i;
                    this.xRot = j;
                }
                ...
                if (this.a > 0 || this.tracker instanceof EntityArrow) {
                    ...
                    if (l2 >= -32768L && l2 < 32768L && i3 >= -32768L && i3 < 32768L && j3 >= -32768L && j3 < 32768L && this.v <= 400 && !this.x && this.y == this.tracker.onGround) {
                        if ((!flag2 || !flag3) && !(this.tracker instanceof EntityArrow)) {
                            if (flag2) {
                                object = new PacketPlayOutEntity.PacketPlayOutRelEntityMove(this.tracker.getId(), l2, i3, j3, this.tracker.onGround);
                            }
                            else if (flag3) {
                                object = new PacketPlayOutEntity.PacketPlayOutEntityLook(this.tracker.getId(), (byte)j2, (byte)k2, this.tracker.onGround);
                            }
                        }
                        else {
                            object = new PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook(this.tracker.getId(), l2, i3, j3, (byte)j2, (byte)k2, this.tracker.onGround);
                        }
                    }
                    else {
                ...
                if (flag4 && this.a > 0) {
                    final double d0 = this.tracker.motX - this.n;
                    final double d2 = this.tracker.motY - this.o;
                    final double d3 = this.tracker.motZ - this.p;
                    final double d4 = 0.02;
                    final double d5 = d0 * d0 + d2 * d2 + d3 * d3;
                    if (d5 > 4.0E-4 || (d5 > 0.0 && this.tracker.motX == 0.0 && this.tracker.motY == 0.0 && this.tracker.motZ == 0.0)) {
                        this.n = this.tracker.motX;
                        this.o = this.tracker.motY;
                        this.p = this.tracker.motZ;
                        this.broadcast((Packet<?>)new PacketPlayOutEntityVelocity(this.tracker.getId(), this.n, this.o, this.p));
                    }
                }
                if (object != null) {
                    if (object instanceof PacketPlayOutEntityTeleport) {
                        ...
                        for (final Map.Entry<EntityPlayer, Boolean> viewer : this.trackedPlayerMap.entrySet()) {
                            if (viewer.getValue()) {
            ...
            final int i = MathHelper.d(this.tracker.getHeadRotation() * 256.0f / 360.0f);
    ...
    private void d() {
        ...
        if (datawatcher.a()) {
            this.broadcastIncludingSelf((Packet<?>)new PacketPlayOutEntityMetadata(this.tracker.getId(), datawatcher, false));
        }
        if (this.tracker instanceof EntityLiving) {
            ...
            if (!set.isEmpty()) {
                if (this.tracker instanceof EntityPlayer) {
                    ((EntityPlayer)this.tracker).getBukkitEntity().injectScaledMaxHealth((Collection)set, false);
                }
                this.broadcastIncludingSelf((Packet<?>)new PacketPlayOutUpdateAttributes(this.tracker.getId(), (Collection)set));
            }
            set.clear();
        }
    }
        ...
        if (entityplayer != this.tracker) {
            if (this.c(entityplayer)) {
                if (!this.trackedPlayers.contains(entityplayer) && (this.e(entityplayer) || this.tracker.attachedToPlayer)) {
                    if (this.tracker instanceof EntityPlayer) {
                        final Player player = (Player)((EntityPlayer)this.tracker).getBukkitEntity();
                    ...
                    if (!this.tracker.getDataWatcher().d()) {
                        entityplayer.playerConnection.sendPacket((Packet)new PacketPlayOutEntityMetadata(this.tracker.getId(), this.tracker.getDataWatcher(), true));
                    }
                    boolean flag = this.u;
                    if (this.tracker instanceof EntityLiving) {
                        final AttributeMapServer attributemapserver = (AttributeMapServer)((EntityLiving)this.tracker).getAttributeMap();
                        final Collection collection = attributemapserver.c();
                        if (this.tracker.getId() == entityplayer.getId()) {
                            ((EntityPlayer)this.tracker).getBukkitEntity().injectScaledMaxHealth(collection, false);
                        }
                        if (!collection.isEmpty()) {
                            entityplayer.playerConnection.sendPacket((Packet)new PacketPlayOutUpdateAttributes(this.tracker.getId(), collection));
                        }
                    ...
                    if (flag && !(packet instanceof PacketPlayOutSpawnEntityLiving)) {
                        entityplayer.playerConnection.sendPacket((Packet)new PacketPlayOutEntityVelocity(this.tracker.getId(), this.tracker.motX, this.tracker.motY, this.tracker.motZ));
                    }
                    if (this.tracker instanceof EntityLiving) {
                        for (final EnumItemSlot enumitemslot : EnumItemSlot.values()) {
                            ...
                            if (!itemstack.isEmpty()) {
                                entityplayer.playerConnection.sendPacket((Packet)new PacketPlayOutEntityEquipment(this.tracker.getId(), enumitemslot, itemstack));
                            }
                        }
                    }
                    if (this.tracker instanceof EntityHuman) {
                        ...
                        if (entityhuman.isSleeping()) {
                            entityplayer.playerConnection.sendPacket((Packet)new PacketPlayOutBed(entityhuman, new BlockPosition(this.tracker)));
                        }
                    }
                    ...
                    if (this.tracker instanceof EntityLiving) {
                        ...
                        for (final MobEffect mobeffect : entityliving.getEffects()) {
                            entityplayer.playerConnection.sendPacket((Packet)new PacketPlayOutEntityEffect(this.tracker.getId(), mobeffect));
                        }
                    }
    ...
    private static boolean hasPassengerInRange(final Entity entity, final EntityPlayer entityplayer) {
        if (!entity.isVehicle()) {
            return false;
        }
        for (final Entity passenger : entity.passengers) {
    ...
    private static boolean isTrackedBy(final Entity entity, final EntityPlayer entityplayer) {
        return entity == entityplayer || (entity.tracker != null && entity.tracker.trackedPlayers.contains(entityplayer));
    }
    
    private void updatePassengers(final EntityPlayer player) {
    ...
    private Packet<?> e() {
        ...
        if (this.tracker instanceof EntityFishingHook) {
            ...
            return (Packet<?>)new PacketPlayOutSpawnEntity(this.tracker, 90, (entityhuman == null) ? this.tracker.getId() : entityhuman.getId());
        }
        if (this.tracker instanceof EntitySpectralArrow) {
            final Entity entity = ((EntitySpectralArrow)this.tracker).shooter;
            return (Packet<?>)new PacketPlayOutSpawnEntity(this.tracker, 91, 1 + ((entity == null) ? this.tracker.getId() : entity.getId()));
        }
        if (this.tracker instanceof EntityTippedArrow) {
            final Entity entity = ((EntityArrow)this.tracker).shooter;
            return (Packet<?>)new PacketPlayOutSpawnEntity(this.tracker, 60, 1 + ((entity == null) ? this.tracker.getId() : entity.getId()));
        }
        ...
        if (this.tracker instanceof EntityFireball) {
            ...
            if (entityfireball.shooter != null) {
                packetplayoutspawnentity = new PacketPlayOutSpawnEntity(this.tracker, (int)b0, ((EntityFireball)this.tracker).shooter.getId());
            }
            else {
        ...
        if (this.tracker instanceof EntityFallingBlock) {
            ...
            return (Packet<?>)new PacketPlayOutSpawnEntity(this.tracker, 70, Block.getCombinedId(entityfallingblock.getBlock()));
        }
        ...
        if (this.tracker instanceof EntityItemFrame) {
            final EntityItemFrame entityitemframe = (EntityItemFrame)this.tracker;
            return (Packet<?>)new PacketPlayOutSpawnEntity(this.tracker, 71, entityitemframe.direction.get2DRotationValue(), entityitemframe.getBlockPosition());
        }
