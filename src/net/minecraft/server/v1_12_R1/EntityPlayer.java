...
public class EntityPlayer extends EntityHuman implements ICrafting
{
    ...
    private void setHasSeenCredits(final boolean has) {
        this.cq = has;
    }
    ...
    public EntityPlayer(final MinecraftServer minecraftserver, final WorldServer worldserver, final GameProfile gameprofile, final PlayerInteractManager playerinteractmanager) {
        super((World)worldserver, gameprofile);
        this.locale = null;
        this.lastSave = MinecraftServer.currentTick;
        this.removeQueue = new ArrayDeque<Integer>();
        ...
        final BlockPosition blockposition = this.getSpawnPoint(minecraftserver, worldserver);
    ...
    public final BlockPosition getSpawnPoint(final MinecraftServer minecraftserver, final WorldServer worldserver) {
        ...
        if (worldserver.worldProvider.m() && worldserver.getWorldData().getGameType() != EnumGamemode.ADVENTURE) {
            ...
            final int j = MathHelper.floor(worldserver.getWorldBorder().b((double)blockposition.getX(), (double)blockposition.getZ()));
            if (j < i) {
                i = j;
            }
            if (j <= 1) {
    ...
    public void a(final NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.hasKeyOfType("enteredNetherPosition", 10)) {
            final NBTTagCompound nbttagcompound2 = nbttagcompound.getCompound("enteredNetherPosition");
            this.cv = new Vec3D(nbttagcompound2.getDouble("x"), nbttagcompound2.getDouble("y"), nbttagcompound2.getDouble("z"));
        }
    ...
    public static void a(final DataConverterManager dataconvertermanager) {
        dataconvertermanager.a(DataConverterTypes.PLAYER, (DataInspector)new DataInspector() {
            public NBTTagCompound a(final DataConverter dataconverter, final NBTTagCompound nbttagcompound, final int i) {
                if (nbttagcompound.hasKeyOfType("RootVehicle", 10)) {
                    final NBTTagCompound nbttagcompound2 = nbttagcompound.getCompound("RootVehicle");
                    if (nbttagcompound2.hasKeyOfType("Entity", 10)) {
                        nbttagcompound2.set("Entity", dataconverter.a((DataConverterType)DataConverterTypes.ENTITY, nbttagcompound2.getCompound("Entity"), i));
                    }
                }
    ...
    public void b(final NBTTagCompound nbttagcompound) {
        ...
        final Entity entity2 = this.bJ();
        if (entity2 != null && entity != this && entity.b(EntityPlayer.class).size() == 1) {
    ...
    public void spawnIn(World world) {
        ...
        if (world == null) {
            ...
            if (world == null || position == null) {
                world = (World)Bukkit.getServer().getWorlds().get(0).getHandle();
    ...
    public void enchantDone(final ItemStack itemstack, final int i) {
    ...
    public void enterCombat() {
        super.enterCombat();
        this.playerConnection.sendPacket((Packet)new PacketPlayOutCombatEvent(this.getCombatTracker(), PacketPlayOutCombatEvent.EnumCombatEventType.ENTER_COMBAT));
    }
    
    public void exitCombat() {
        super.exitCombat();
        this.playerConnection.sendPacket((Packet)new PacketPlayOutCombatEvent(this.getCombatTracker(), PacketPlayOutCombatEvent.EnumCombatEventType.END_COMBAT));
    }
    ...
    public void B_() {
        ...
        while (!this.removeQueue.isEmpty()) {
            final int i = Math.min(this.removeQueue.size(), Integer.MAX_VALUE);
        ...
        final Entity entity = this.getSpecatorTarget();
        if (entity != this) {
            if (entity.isAlive()) {
        ...
        try {
            ...
            for (int i = 0; i < this.inventory.getSize(); ++i) {
                ...
                if (!itemstack.isEmpty() && itemstack.getItem().f()) {
                    final Packet packet = ((ItemWorldMapBase)itemstack.getItem()).a(itemstack, this.world, (EntityHuman)this);
                    if (packet != null) {
            ...
            if (this.getHealth() != this.lastHealthSent || this.ch != this.foodData.getFoodLevel() || this.foodData.getSaturationLevel() == 0.0f != this.ci) {
                this.playerConnection.sendPacket((Packet)new PacketPlayOutUpdateHealth(this.getBukkitEntity().getScaledHealth(), this.foodData.getFoodLevel(), this.foodData.getSaturationLevel()));
    ...
    private void a(final IScoreboardCriteria iscoreboardcriteria, final int i) {
        final Collection collection = this.world.getServer().getScoreboardManager().getScoreboardScores(iscoreboardcriteria, this.getName(), (Collection)new ArrayList());
    ...
    public void die(final DamageSource damagesource) {
        ...
        if (this.dead) {
            this.playerConnection.sendPacket((Packet)new PacketPlayOutCombatEvent(this.getCombatTracker(), PacketPlayOutCombatEvent.EnumCombatEventType.ENTITY_DIED, flag));
            return;
        }
        final List<org.bukkit.inventory.ItemStack> loot = new ArrayList<org.bukkit.inventory.ItemStack>(this.inventory.getSize());
        final boolean keepInventory = this.world.getGameRules().getBoolean("keepInventory") || this.isSpectator();
        if (!keepInventory) {
            for (final ItemStack item : this.inventory.getContents()) {
        ...
        final IChatBaseComponent chatmessage = this.getCombatTracker().getDeathMessage();
        final String deathmessage = chatmessage.toPlainText();
        final PlayerDeathEvent event = CraftEventFactory.callPlayerDeathEvent(this, (List)loot, deathmessage, keepInventory);
        ...
        if (deathMessage != null && deathMessage.length() > 0 && flag) {
            if (deathMessage.equals(deathmessage)) {
                final ScoreboardTeamBase scoreboardteambase = this.aY();
        ...
        this.setSpectatorTarget((Entity)this);
        final Collection collection = this.world.getServer().getScoreboardManager().getScoreboardScores(IScoreboardCriteria.d, this.getName(), (Collection)new ArrayList());
        for (final ScoreboardScore scoreboardscore : collection) {
            scoreboardscore.incrementScore();
        }
        final EntityLiving entityliving = this.ci();
        if (entityliving != null) {
            final EntityTypes.MonsterEggInfo entitytypes_monsteregginfo = EntityTypes.eggInfo.get(EntityTypes.a((Entity)entityliving));
    ...
    public void a(final Entity entity, final int i, final DamageSource damagesource) {
        if (entity != this) {
            ...
            this.addScore(i);
            final Collection<ScoreboardScore> collection = (Collection<ScoreboardScore>)this.world.getServer().getScoreboardManager().getScoreboardScores(IScoreboardCriteria.f, this.getName(), (Collection)new ArrayList());
            if (entity instanceof EntityHuman) {
                this.b(StatisticList.D);
                this.world.getServer().getScoreboardManager().getScoreboardScores(IScoreboardCriteria.e, this.getName(), (Collection)collection);
            }
            else {
    ...
    private Collection<ScoreboardScore> E(final Entity entity) {
        final String s = (entity instanceof EntityHuman) ? entity.getName() : entity.bn();
        final ScoreboardTeam scoreboardteam = this.getScoreboard().getPlayerTeam(this.getName());
        if (scoreboardteam != null) {
            final int i = scoreboardteam.getColor().b();
            if (i >= 0 && i < IScoreboardCriteria.n.length) {
                for (final ScoreboardObjective scoreboardobjective : this.getScoreboard().getObjectivesForCriteria(IScoreboardCriteria.n[i])) {
                    final ScoreboardScore scoreboardscore = this.getScoreboard().getPlayerScoreForObjective(s, scoreboardobjective);
                    scoreboardscore.incrementScore();
                }
            }
        }
        final ScoreboardTeam scoreboardteam2 = this.getScoreboard().getPlayerTeam(s);
        if (scoreboardteam2 != null) {
            final int j = scoreboardteam2.getColor().b();
            if (j >= 0 && j < IScoreboardCriteria.m.length) {
                return (Collection<ScoreboardScore>)this.world.getServer().getScoreboardManager().getScoreboardScores(IScoreboardCriteria.m[j], this.getName(), (Collection)new ArrayList());
            }
        }
        return (Collection<ScoreboardScore>)Lists.newArrayList();
    }
    ...
    public boolean damageEntity(final DamageSource damagesource, final float f) {
        ...
        final boolean flag = this.server.aa() && this.canPvP() && "fall".equals(damagesource.translationIndex);
    ...
    public EntityHuman.EnumBedResult a(final BlockPosition blockposition) {
        final EntityHuman.EnumBedResult entityhuman_enumbedresult = super.a(blockposition);
        if (entityhuman_enumbedresult == EntityHuman.EnumBedResult.OK) {
            this.b(StatisticList.ab);
            final PacketPlayOutBed packetplayoutbed = new PacketPlayOutBed((EntityHuman)this, blockposition);
    ...
    public void a(final boolean flag, final boolean flag1, final boolean flag2) {
        ...
        if (this.isSleeping()) {
            this.x().getTracker().sendPacketToEntity((Entity)this, (Packet<?>)new PacketPlayOutAnimation((Entity)this, 2));
        }
    ...
    public void stopRiding() {
        ...
        if (entity instanceof EntityPlayer) {
            final WorldServer worldServer = (WorldServer)entity.getWorld();
    ...
    protected void a(final double d0, final boolean flag, final IBlockData iblockdata, final BlockPosition blockposition) {
    }
    
    protected void b(final BlockPosition blockposition) {
        if (!this.isSpectator()) {
            super.b(blockposition);
        }
    }
    
    public void a(final double d0, final boolean flag) {
        ...
        if (iblockdata.getMaterial() == Material.AIR) {
            ...
            final Block block = iblockdata2.getBlock();
    ...
    public void openSign(final TileEntitySign tileentitysign) {
    ...
    public void openTileEntity(final ITileEntityContainer itileentitycontainer) {
        final boolean cancelled = itileentitycontainer instanceof ILootable && ((ILootable)itileentitycontainer).b() != null && this.isSpectator();
        final Container container = CraftEventFactory.callInventoryOpenEvent(this, itileentitycontainer.createContainer(this.inventory, (EntityHuman)this), cancelled);
        if (container == null) {
            return;
        }
        this.nextContainerCounter();
        this.activeContainer = container;
        this.playerConnection.sendPacket((Packet)new PacketPlayOutOpenWindow(this.containerCounter, itileentitycontainer.getContainerName(), itileentitycontainer.getScoreboardDisplayName()));
    ...
    public void openContainer(final IInventory iinventory) {
        ...
        if (iinventory instanceof ITileInventory) {
            final ITileInventory itileinventory = (ITileInventory)iinventory;
            cancelled = (itileinventory.isLocked() && !this.a(itileinventory.getLock()) && !this.isSpectator());
        }
        Container container;
        if (iinventory instanceof ITileEntityContainer) {
            if (iinventory instanceof TileEntity) {
                Preconditions.checkArgument(((TileEntity)iinventory).getWorld() != null, (Object)"Container must have world to be opened");
            }
        ...
        if (iinventory instanceof ILootable && ((ILootable)iinventory).b() != null && this.isSpectator()) {
            ...
            if (iinventory instanceof ITileInventory) {
                final ITileInventory itileinventory2 = (ITileInventory)iinventory;
                if (itileinventory2.isLocked() && !this.a(itileinventory2.getLock()) && !this.isSpectator()) {
                    this.playerConnection.sendPacket((Packet)new PacketPlayOutChat((IChatBaseComponent)new ChatMessage("container.isLocked", new Object[] { iinventory.getScoreboardDisplayName() }), ChatMessageType.GAME_INFO));
                    this.playerConnection.sendPacket((Packet)new PacketPlayOutNamedSoundEffect(SoundEffects.ab, SoundCategory.BLOCKS, this.locX, this.locY, this.locZ, 1.0f, 1.0f));
                    iinventory.closeContainer((EntityHuman)this);
                    return;
                }
            }
            ...
            if (iinventory instanceof ITileEntityContainer) {
                this.activeContainer = container;
                this.playerConnection.sendPacket((Packet)new PacketPlayOutOpenWindow(this.containerCounter, ((ITileEntityContainer)iinventory).getContainerName(), iinventory.getScoreboardDisplayName(), iinventory.getSize()));
            }
            else {
                this.activeContainer = container;
                this.playerConnection.sendPacket((Packet)new PacketPlayOutOpenWindow(this.containerCounter, "minecraft:container", iinventory.getScoreboardDisplayName(), iinventory.getSize()));
            }
            this.activeContainer.windowId = this.containerCounter;
            this.activeContainer.addSlotListener((ICrafting)this);
        }
    ...
    public void openTrade(final IMerchant imerchant) {
        final Container container = CraftEventFactory.callInventoryOpenEvent(this, (Container)new ContainerMerchant(this.inventory, imerchant, this.world));
        if (container == null) {
            return;
        }
        this.nextContainerCounter();
        ...
        this.activeContainer.windowId = this.containerCounter;
        this.activeContainer.addSlotListener((ICrafting)this);
        final InventoryMerchant inventorymerchant = ((ContainerMerchant)this.activeContainer).e();
        final IChatBaseComponent ichatbasecomponent = imerchant.getScoreboardDisplayName();
        this.playerConnection.sendPacket((Packet)new PacketPlayOutOpenWindow(this.containerCounter, "minecraft:villager", ichatbasecomponent, inventorymerchant.getSize()));
        final MerchantRecipeList merchantrecipelist = imerchant.getOffers((EntityHuman)this);
        if (merchantrecipelist != null) {
            ...
            this.playerConnection.sendPacket((Packet)new PacketPlayOutCustomPayload("MC|TrList", packetdataserializer));
        }
    }
    
    public void openHorseInventory(final EntityHorseAbstract entityhorseabstract, final IInventory iinventory) {
        final Container container = CraftEventFactory.callInventoryOpenEvent(this, (Container)new ContainerHorse((IInventory)this.inventory, iinventory, entityhorseabstract, (EntityHuman)this));
        if (container == null) {
            iinventory.closeContainer((EntityHuman)this);
            return;
        }
        ...
        this.nextContainerCounter();
        this.playerConnection.sendPacket((Packet)new PacketPlayOutOpenWindow(this.containerCounter, "EntityHorse", iinventory.getScoreboardDisplayName(), iinventory.getSize(), entityhorseabstract.getId()));
        this.activeContainer = container;
        this.activeContainer.windowId = this.containerCounter;
        this.activeContainer.addSlotListener((ICrafting)this);
    }
    
    public void a(final ItemStack itemstack, final EnumHand enumhand) {
    ...
    public void a(final TileEntityCommand tileentitycommand) {
    ...
    public void a(final Container container, final NonNullList<ItemStack> nonnulllist) {
        this.playerConnection.sendPacket((Packet)new PacketPlayOutWindowItems(container.windowId, (NonNullList)nonnulllist));
        ...
        if (EnumSet.of(InventoryType.CRAFTING, InventoryType.WORKBENCH).contains(container.getBukkitView().getType())) {
            this.playerConnection.sendPacket((Packet)new PacketPlayOutSetSlot(container.windowId, 0, container.getSlot(0).getItem()));
        }
    }
    ...
    public void setContainerData(final Container container, final IInventory iinventory) {
        for (int i = 0; i < iinventory.h(); ++i) {
            this.playerConnection.sendPacket((Packet)new PacketPlayOutWindowData(container.windowId, i, iinventory.getProperty(i)));
        }
    }
    ...
    public void a(final float f, final float f1, final boolean flag, final boolean flag1) {
        if (this.isPassenger()) {
            if (f >= -1.0f && f <= 1.0f) {
                this.be = f;
            }
    ...
    public void a(final Statistic statistic, final int i) {
        if (statistic != null) {
            ...
            for (final ScoreboardObjective scoreboardobjective : this.getScoreboard().getObjectivesForCriteria(statistic.f())) {
    ...
    public void a(final List<IRecipe> list) {
        this.cr.a((List)list, this);
    }
    
    public void a(final MinecraftKey[] aminecraftkey) {
        final ArrayList arraylist = Lists.newArrayList();
        for (final MinecraftKey minecraftkey : aminecraftkey) {
    ...
    public void b(final List<IRecipe> list) {
    ...
    public void sendMessage(final IChatBaseComponent[] ichatbasecomponent) {
        for (final IChatBaseComponent component : ichatbasecomponent) {
            this.sendMessage(component);
        }
    }
    
    public void a(final IChatBaseComponent ichatbasecomponent, final boolean flag) {
        this.playerConnection.sendPacket((Packet)new PacketPlayOutChat(ichatbasecomponent, flag ? ChatMessageType.GAME_INFO : ChatMessageType.CHAT));
    }
    ...
    public void copyFrom(final EntityPlayer entityplayer, final boolean flag) {
        if (flag) {
        ...
        if (this.removeQueue != entityplayer.removeQueue) {
            this.removeQueue.addAll((Collection<?>)entityplayer.removeQueue);
        }
    ...
    protected void a(final MobEffect mobeffect, final boolean flag) {
    ...
    protected void b(final MobEffect mobeffect) {
        super.b(mobeffect);
        this.playerConnection.sendPacket((Packet)new PacketPlayOutRemoveEntityEffect(this.getId(), mobeffect.getMobEffect()));
        if (mobeffect.getMobEffect() == MobEffects.LEVITATION) {
            this.cs = null;
        }
        CriterionTriggers.z.a(this);
    ...
    public void a(final EnumGamemode enumgamemode) {
        ...
        final PlayerGameModeChangeEvent event = new PlayerGameModeChangeEvent((Player)this.getBukkitEntity(), GameMode.getByValue(enumgamemode.getId()));
    ...
    public void a(final PacketPlayInSettings packetplayinsettings) {
        if (this.getMainHand() != packetplayinsettings.getMainHand()) {
            final PlayerChangedMainHandEvent event = new PlayerChangedMainHandEvent((Player)this.getBukkitEntity(), (this.getMainHand() == EnumMainHand.LEFT) ? MainHand.LEFT : MainHand.RIGHT);
        ...
        if (!oldLocale.equals(packetplayinsettings.a())) {
            final org.bukkit.event.player.PlayerLocaleChangeEvent event2 = new org.bukkit.event.player.PlayerLocaleChangeEvent((Player)this.getBukkitEntity(), packetplayinsettings.a());
            this.server.server.getPluginManager().callEvent((Event)event2);
        }
        this.cl = packetplayinsettings.c();
        this.cm = packetplayinsettings.d();
        this.getDataWatcher().set(EntityPlayer.br, (Object)(byte)packetplayinsettings.e());
        this.getDataWatcher().set(EntityPlayer.bs, (Object)(byte)((packetplayinsettings.getMainHand() != EnumMainHand.LEFT) ? 1 : 0));
    }
    ...
    public void setSpectatorTarget(final Entity entity) {
        final Entity entity2 = this.getSpecatorTarget();
        this.co = (Entity)((entity == null) ? this : entity);
        if (entity2 != this.co) {
            this.playerConnection.sendPacket((Packet)new PacketPlayOutCamera(this.co));
            this.playerConnection.a(this.co.locX, this.co.locY, this.co.locZ, this.yaw, this.pitch, PlayerTeleportEvent.TeleportCause.SPECTATE);
        }
    }
    ...
    public void a(final EnumHand enumhand) {
        super.a(enumhand);
        this.ds();
    }
    ...
    public void forceSetPositionRotation(final double x, final double y, final double z, final float yaw, final float pitch) {
