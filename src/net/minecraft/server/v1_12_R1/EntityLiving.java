...
//		AsyncCatcher.catchOp((String) "effect add");
		...
		if (!this.isInvulnerable(damagesource)) {
			...
			EntityDamageEvent event = CraftEventFactory.handleLivingEntityDamageEvent((Entity) this,
					...
					(double) (absorptionModifier = ((absorption = new Function<Double, Double>() {
						...
						public Double apply(Double f) {
							return -Math.max(f - Math.max(f - (double) EntityLiving.this.getAbsorptionHearts(), 0.0),
									0.0);
						}
					}).apply((double) (f += magicModifier))).floatValue()), hardHat, blocking, armor, resistance, magic,
					absorption);
			if (event.isCancelled()) {
				return false;
			}
			...
			if ((damagesource == DamageSource.ANVIL || damagesource == DamageSource.FALLING_BLOCK)
				...
				this.getEquipment(EnumItemSlot.HEAD).damage(
						(int) (event.getDamage() * 4.0 + (double) this.random.nextFloat() * event.getDamage() * 2.0),
						this);
			}
			if (!damagesource.ignoresArmor()) {
	...
	public void B_() {
		...
		if (!this.world.isClientSide) {
			...
			block8: for (EnumItemSlot enumitemslot : EnumItemSlot.values()) {
				ItemStack itemstack;
