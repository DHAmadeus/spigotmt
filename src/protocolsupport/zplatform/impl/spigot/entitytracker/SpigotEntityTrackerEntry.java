...
public class SpigotEntityTrackerEntry extends EntityTrackerEntry {
	...
	public SpigotEntityTrackerEntry(Entity entity, int trackRange, int viewDistance, int updateInterval,
	...
	protected void updateRotationIfChanged() {
		...
		if (Math.abs(eYaw - lastYaw) >= 1 || Math.abs(ePitch - lastPitch) >= 1) {
			lastYaw = eYaw;
			lastPitch = ePitch;
			broadcast(new PacketPlayOutEntity.PacketPlayOutEntityLook(entity.getId(),
					(byte) MathHelper.d((eYaw * 256.0f) / 360.0f), (byte) MathHelper.d((ePitch * 256.0f) / 360.0f),
					entity.onGround));
		}
	}

	@Override
	public synchronized void track(List<EntityHuman> worldPlayers) {
		b = false;
		...
		if (((a % 20) == 0) && entity instanceof EntityItemFrame) {
		...
		if (((a > 0) && (a % updateInterval) == 0) || entity.impulse) {
			...
			if (entity.isPassenger()) {
				...
				if (Math.abs(entity.locX - lastLocX) >= 0.03125D || Math.abs(entity.locY - lastLocY) >= 0.015625D
				...
				if (updateVelocity) {
					...
					if ((diffMot > 4.0E-4) || ((diffMot > 0.0) && (entity.motX == 0.0) && (entity.motY == 0.0)
							&& (entity.motZ == 0.0))) {
			...
			if (Math.abs(eHeadYaw - lastHeadYaw) >= 1) {
				...
				broadcast(
						new PacketPlayOutEntityHeadRotation(entity, (byte) MathHelper.d((eHeadYaw * 256.0f) / 360.0f)));
			}
		}
		...
		if (entityplayer != entity) {
			if (c(entityplayer)) {
				if (!trackedPlayers.contains(entityplayer)
						&& (canPlayerSeeTrackerChunk(entityplayer) || entity.attachedToPlayer)) {
					if (entity instanceof EntityPlayer) {
					...
					broadcast(new PacketPlayOutEntityHeadRotation(entity,
							(byte) MathHelper.d((lastHeadYaw * 256.0f) / 360.0f)));
					...
					if (entity instanceof EntityLiving) {
						...
						Collection<AttributeInstance> updateAttrs = ((AttributeMapServer) entityliving
								.getAttributeMap()).c();
						...
						if (!updateAttrs.isEmpty()) {
							entityplayer.playerConnection
									.sendPacket(new PacketPlayOutUpdateAttributes(entity.getId(), updateAttrs));
						}
						for (EnumItemSlot enumitemslot : EnumItemSlot.values()) {
							ItemStack itemstack = entityliving.getEquipment(enumitemslot);
							if (!itemstack.isEmpty()) {
								entityplayer.playerConnection.sendPacket(
										new PacketPlayOutEntityEquipment(entity.getId(), enumitemslot, itemstack));
							}
						}
						for (MobEffect mobeffect : entityliving.getEffects()) {
							entityplayer.playerConnection
									.sendPacket(new PacketPlayOutEntityEffect(entity.getId(), mobeffect));
						}
					}
					if (updateVelocity && !(spawnPacket instanceof PacketPlayOutSpawnEntityLiving)) {
						...
						lastMotZ = entity.motZ;
						entityplayer.playerConnection.sendPacket(
								new PacketPlayOutEntityVelocity(entity.getId(), entity.motX, entity.motY, entity.motZ));
					}
					if (entity instanceof EntityHuman) {
						EntityHuman entityhuman = (EntityHuman) entity;
						if (entityhuman.isSleeping()) {
							entityplayer.playerConnection
									.sendPacket(new PacketPlayOutBed(entityhuman, new BlockPosition(entity)));
						}
					}
	...
	protected synchronized void addTrackedPlayer(EntityPlayer entityplayer) {
	...
	protected synchronized boolean removeTrackedPlayer(EntityPlayer entityplayer) {
		if (paperTrackedPlayersMapPresent) {
	...
	public boolean c(EntityPlayer entityplayer) {
		...
		return (diffX >= -lTrackRange) && (diffX <= lTrackRange) && (diffZ >= -lTrackRange) && (diffZ <= lTrackRange)
				&& entity.a(entityplayer);
	}
	...
	static {
		createSpawnPacketMethods.setKnownPath(EntityPlayer.class,
				entity -> new PacketPlayOutNamedEntitySpawn((EntityHuman) entity));
		createSpawnPacketMethods.setKnownPath(IAnimal.class,
				entity -> new PacketPlayOutSpawnEntityLiving((EntityLiving) entity));
		createSpawnPacketMethods.setKnownPath(EntityPainting.class,
				entity -> new PacketPlayOutSpawnEntityPainting((EntityPainting) entity));
		...
		createSpawnPacketMethods.setKnownPath(EntityExperienceOrb.class,
				entity -> new PacketPlayOutSpawnEntityExperienceOrb((EntityExperienceOrb) entity));
		createSpawnPacketMethods.setKnownPath(EntityFishingHook.class, entity -> {
			EntityHuman entityhuman = ((EntityFishingHook) entity).l();
			return new PacketPlayOutSpawnEntity(entity, 90,
					(entityhuman == null) ? entity.getId() : entityhuman.getId());
		});
		...
		createSpawnPacketMethods.setKnownPath(EntityLlamaSpit.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 68));
		createSpawnPacketMethods.setKnownPath(EntityPotion.class, entity -> new PacketPlayOutSpawnEntity(entity, 73));
		createSpawnPacketMethods.setKnownPath(EntityThrownExpBottle.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 75));
		createSpawnPacketMethods.setKnownPath(EntityEnderPearl.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 65));
		createSpawnPacketMethods.setKnownPath(EntityEnderSignal.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 72));
		createSpawnPacketMethods.setKnownPath(EntityFireworks.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 76));
		...
		createSpawnPacketMethods.setKnownPath(EntityEvokerFangs.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 79));
		createSpawnPacketMethods.setKnownPath(EntityTNTPrimed.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 50));
		createSpawnPacketMethods.setKnownPath(EntityEnderCrystal.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 51));
		...
		createSpawnPacketMethods.setKnownPath(EntityArmorStand.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 78));
		createSpawnPacketMethods.setKnownPath(EntityItemFrame.class, entity -> {
			EntityItemFrame entityitemframe = (EntityItemFrame) entity;
			return new PacketPlayOutSpawnEntity(entity, 71, entityitemframe.direction.get2DRotationValue(),
		...
		createSpawnPacketMethods.setKnownPath(EntityAreaEffectCloud.class,
				entity -> new PacketPlayOutSpawnEntity(entity, 3));
